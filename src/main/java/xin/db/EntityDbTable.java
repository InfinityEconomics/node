...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion && height < Xin.getBlockchainProcessor().getMinRollbackHeight()) {
            throw new IllegalArgumentException("Historical data as of height " + height + " not available.");
        }
        if (height > Xin.getBlockchain().getHeight()) {
            throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Xin.getBlockchain().getHeight());
        }
    }
    ...
    public final DbIterator<T> getManyBy(DbClause dbClause, int from, int to, String sortClause) {
        Connection con = null;
        try {
            con = db.getConnection();
            PreparedStatement pstmt = con.prepareStatement("SELECT * FROM " + table
                    + " WHERE " + dbClause.getClause() + (multiversion ? " AND latest = TRUE " : " ") + sortClause
    ...
    public final DbIterator<T> getManyBy(DbClause dbClause, int height, int from, int to, String sortClause) {
        if (height < 0 || height == Xin.getBlockchain().getHeight()) {
            return getManyBy(dbClause, from, to, sortClause);
        }
        checkAvailable(height);
        Connection con = null;
        try {
